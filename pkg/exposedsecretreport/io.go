...
type Writer interface {
	Write(context.Context, []v1alpha1.ExposedSecretReport, []v1alpha1.ClusterExposedSecretReport) error
}
...
func (r *readWriter) Write(ctx context.Context, reports []v1alpha1.ExposedSecretReport, clusterReports []v1alpha1.ClusterExposedSecretReport) error {
	...
	for _, report := range clusterReports {
		err := r.createOrUpdateClusterReport(ctx, report)
		if err != nil {
			return err
		}
	}
...
func (r *readWriter) createOrUpdateClusterReport(ctx context.Context, report v1alpha1.ClusterExposedSecretReport) error {
	var existing v1alpha1.ClusterExposedSecretReport
	err := r.Get(ctx, types.NamespacedName{
		Name: report.Name,
	}, &existing)
